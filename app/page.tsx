...
function PostCard({
                      ...
                      reportingPost, reportReason, postActionMsg,
...
export default function Home() {
    ...
    const handleJoin = async (post: NearbyPost) => {
        ...
        try {
            // Already accepted → open chat directly
            const accepted = await getAcceptedRequest(post.id, user.uid);
            if (accepted) { router.push(`/chat/${post.id}_${user.uid}`); return; }
